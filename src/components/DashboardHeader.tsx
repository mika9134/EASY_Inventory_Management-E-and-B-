...
import { PlusCircle, ArrowUpRight } from 'lucide-react'; // Swapped to PlusCircle
    ...
    <>
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in">
        ...
        <div className="flex items-center gap-3">
          {/* Stock In: Updated to a vibrant teal/emerald look */}
          <Button
            onClick={() => setIsStockInOpen(true)}
            className="h-14 px-8 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl shadow-xl shadow-emerald-600/20 hover:scale-105 transition-all text-lg font-bold"
          >
            <PlusCircle className="w-6 h-6 mr-2" /> Stock In
          </Button>

          {/* Stock Out: Kept danger styling for contrast */}
          <Button
            variant="danger"
